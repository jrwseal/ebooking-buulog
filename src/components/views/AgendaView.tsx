      ...
      {grouped.map(([date, items]) => (
        <div key={date} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-100 font-semibold text-sm text-slate-700">
            {thaiFull(date)}{' '}
            {date === todayStr && <span className="text-buu">· วันนี้</span>}
          </div>
            ...
            {items.map((b) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left border border-slate-200 rounded-lg p-2.5 hover:border-buu-subtle transition"
                >
